...
#   turtle.json (dataServiceHarvester/turtle)
#   fdkServices.json (dataServiceHarvester/fdkServices)
#   dataserviceMeta.json (dataServiceHarvester/dataserviceMeta)
#
...
for meta in $(cat dataserviceMeta.json | jq -c .[]); do
    ...
    turtle_id=$(cat turtle_ids.txt | grep "dataservice-$fdkid")
...
for action in $(cat actions_sorted); do
    ...
    if [[ "$actiontype" == "create" ]]; then
        cat fdkServices.json | jq -r ".[] | select(._id == \"$fdkid\") | .turtle" | base64 -d | gunzip -f - \
