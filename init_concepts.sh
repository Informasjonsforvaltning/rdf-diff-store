...
#   turtle.json (conceptHarvester/turtle)
#   concepts.json (conceptHarvester/fdkConcepts)
#   conceptMeta.json (conceptHarvester/conceptMeta)
#
...
for meta in $(cat conceptMeta.json | jq -c .[]); do
    ...
    turtle_id=$(cat turtle_ids.txt | grep "concept-$fdkid")


    if [[ "$uri" != "" ]] && [[ "$fdkid" != "" ]] && [[ "$catalog" != "" ]] && [[ "$turtle_id" != "" ]] && [[ "$issued" != "" ]] && [[ "$modified" != "" ]]; then
        catalog_turtle_id=collection-$(echo $catalog | cut -d "/" -f5)
...
for action in $(cat actions_sorted); do
    ...
    if [[ "$actiontype" == "create" ]]; then
        cat concepts.json | jq -r ".[] | select(._id == \"$fdkid\") | .turtle" | base64 -d | gunzip -f - \
        ...
        | base64 \
        | python -c "import sys; from base64 import b64decode; from rdflib import Graph; print(Graph().parse(data=b64decode(sys.stdin.read()), format=\"text/turtle\").serialize(format=\"text/turtle\").rstrip(), end='')" \
